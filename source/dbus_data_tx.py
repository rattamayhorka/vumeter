...
# Configura el nivel de registro y el formato del mensaje de registro
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filename='daemon.log')

logging.info("Configurando el puerto serie")
...
def obtener_reproductores_activos():
    try:
        ...
        if not player_lines:
            logging.info("No se encontraron reproductores multimedia activos.")
        ...
        logging.error(f"Error al obtener reproductores activos: {e}")
...
def recopilar_data():
    ...
    while True:
        try:
            ...
            if not player_lines:
                logging.info("recopilar_data No hay reproductores activos. Esperando...")
            ...
            if title and service_name and (contador < time_until_print_pc_vars):
                ...
                if max_len > 21: # Verifica si max_len es mayor a 20
                    for i in range(max_len - 20):  # Ajuste aquí
                        ...
                        output = f"\a{linea_1}\n{linea_2}\n{linea_3}\n{linea_4}\n"
                        logging.info(f"enviando datos mayores de 20 caracteres... contador: {contador}")
                        send_serial(output)
                        time.sleep(0.3)
                    ...
                    title = ""
                    reproductor = ""
                    logging.info(f"enviando datos menores de 20 caracteres... contador: {contador}")
            ...
            if contador >= time_until_print_pc_vars:
                ...
                logging.info(f"inicio de impresion de variables pc: {contador}")
    
            if contador >= time_until_print_pc_vars + wait_printing_pc_vars:
                contador = 0
                logging.info(f"fin de impresion de variables: {contador}")
    
            send_serial(output)

        except Exception as e:
            if "No player is being controlled by playerctld" in str(e):
                logging.error("exception No hay reproductor multimedia activo siendo controlado por playerctld.")
                output = f"\aEsperando\nReproductor...\n\n\n"
                send_serial(output)
            else:
                logging.error(f"Error Exception: {e}")  # Maneja otras excepciones
        ...
        while True:
            ...
            if char_value == '\n':
                if "-" in char_buffer:
                    ...
                    if "-i" in command:
                        logging.info("received command increment volume")
                    elif "-d" in command:
                        logging.info("received command decrement volume")
                    elif "-t" in command:
                        logging.info("received command mute/unmute")
                    char_buffer = ""  # Limpia el buffer
                elif "prev" in char_buffer:
                    logging.info("recived prev command")
                    subprocess.run(["playerctl", "previous"])
                    char_buffer = ""  # Limpia el buffer
                elif "next" in char_buffer:
                    logging.info("received next command")
